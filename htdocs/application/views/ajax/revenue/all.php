...
<!-- ------------------------------------------------------------------------------------------------------------------------------- -->
<!-- DANH SACH DON HANG THANH CONG -->
<div style="text-align: center;">
    <label style="color: #428bca;font-size: 23px;margin-top: 7px;margin-bottom: 15px;">Danh sách đơn hàng thành công</label>
</div>
<table class="table table-bordered table-striped">
    ...
    <?php if (isset($_list_orders) && count($_list_orders)) :
        foreach ($_list_orders as $key => $item) :
                // echo "<br>".json_encode($item)."<br>";
                // echo gettype($item['detail_order']);//string
                // echo gettype($item['deleted']);//string

                if($item['deleted'] != 0) { // deleted = 0 la hoa don khong huy or xoa
                    continue;
                }
...
<div style="text-align: center;">
    <label style="color: red;font-size: 23px;margin-top: 20px;margin-bottom: 14px;">Danh sách đơn hàng hủy</label>
</div>
<table class="table table-bordered table-striped">
    <thead>
    <tr>
        ...
        <th class="text-center" style="background-color: #fff;">Lý do hủy</th>
    </tr>
    </thead>
    <tbody>
    <?php if (isset($_list_orders) && count($_list_orders)) :
        foreach ($_list_orders as $key => $item) :
            ...
            // echo "<br>".json_encode($item)."<br>";
            if($item['deleted'] == 0) {// deleted = 1 la hoa don bi huy, bi xoa
                continue;
            }
            ...
            <tr>
                ...
                <td class="text-center"><?php echo cms_getNameAuthbyID($item['user_init']); ?></td>
                <td class="text-center"><?php echo cms_getNamecustomerbyID($item['customer_id']); ?></td>
                <td class="text-center"><?php echo ($item['sell_date'] != '0000-00-00 00:00:00') ? gmdate("H:i d/m/Y", strtotime(str_replace('-', '/', $item['updated'])) + 7 * 3600) : '-'; ?></td>
                <td class="text-center"
                    style="background-color: #F2F2F2;"><b><?php echo $item['notes']; ?></b></td>
            </tr>
            <tr class="tr-hide" id="tr-detail-order-<?php echo $item['ID']?>">
                <td colspan="15">
                    <div class="tabbable">
